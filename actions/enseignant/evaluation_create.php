...
error_log("--- EVALUATION CREATE START ---");
...
error_log("Params: CID=$cours_id, LID=$lecon_id, Title=$titre");
...
if ($coursModel->appartientA($cours_id, $_SESSION['user_id'])) {
    ...
    $existingEval = $evaluationModel->trouverParLecon($lecon_id);
    if ($existingEval) {
        error_log("EVAL ALREADY EXISTS FOR LESSON $lecon_id");
        set_flash('error', 'Une évaluation existe déjà pour cette leçon.');
        rediriger('enseignant/evaluation_edit', ['id' => $existingEval['id']]);
    } else {
        error_log("CREATING NEW EVAL");
        try {
            ...
            set_flash('success', 'Évaluation créée.');
            error_log("EVAL CREATED SUCCESSFULLY: $eval_id");
        } catch (Exception $e) {
            error_log("DB CREATION FAILED: " . $e->getMessage());
            set_flash('error', 'Erreur BDD: ' . $e->getMessage());
        }
    }
} else {
